...
class Decoder_Dual(nn.Module):
    def __init__(self, params):
        ...
        self.out1_conv = nn.Conv2d(self.ft_chns[0], self.n_class,
                                  kernel_size=3, padding=1)
        self.out2_conv = nn.Conv2d(self.ft_chns[0], self.n_class,
                                  kernel_size=3, padding=1)
    ...
    def forward(self, feature):
        ...
        x = self.up3(x, x1)
        x = self.up4(x, x0)
        output1 = self.out1_conv(x)
        output2 = self.out2_conv(x)
...
class UNet_Share_Encoder(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_Share_Encoder, self).__init__()
        ...
        self.encoder = Encoder(params)
        self.decoder1 = Decoder_Dual(params)
        self.decoder2 = Decoder_Dual(params)
...
class UNet_Dual(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_Dual(params)
